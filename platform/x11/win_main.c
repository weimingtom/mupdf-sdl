...
{
	timer_pending = 1;
	SetTimer(hwndview, OUR_TIMER_ID, (unsigned int)(1000*duration), NULL);
}
...
void handlekey(int c)
{
	...
	if (c > 256)
	{
		...
		case VK_PRIOR: c = ','; break; //PageUp
		case VK_NEXT: c = '.'; break; //PageDown
		}
	}
...
#if defined(__MINGW32__)
	while ((c = fz_getopt(argc, argv, "p:r:A:C:W:H:S:U:b:z:")) != -1)
#else
...
#if defined(__MINGW32__)		
        case 'z': zarg = fz_atoi(fz_optarg); break;
#endif		
		default: usage();
		}
	}
