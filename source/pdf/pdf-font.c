...
pdf_load_simple_font_by_name(fz_context *ctx, pdf_document *doc, pdf_obj *dict, char *basefont)
{
...
#if defined(__MINGW32__)
    fz_warn(ctx, "===basefont: %s", basefont);
    //if (basefont != NULL && basefont[0] == 'A') {
    //    fz_warn(ctx, "===basefont: %s bingo", basefont);
    //}
#endif	
	/* Load font file */
	fz_try(ctx)
	{
        ...
        //fz_warn(ctx, "===basefont: %s bingo 222", basefont);
#endif		
		if (cmap)
		{
